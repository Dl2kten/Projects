...
/*
* Exercise 15.26
* Define the Quote and Bulk_quote copy-control members to do same job
* as synthesized, given them print statements
*/
...
public:
	...
	Bulk_quote(const Bulk_quote& rhs) : Disc_quote(rhs) {
		std::cout << "Bult_quote copy constructor" << std::endl;
	}
	Bulk_quote(Bulk_quote&& rhs) noexcept : Disc_quote(std::move(rhs)){
		std::cout << "Bult_quote move constructor" << std::endl;
	}
