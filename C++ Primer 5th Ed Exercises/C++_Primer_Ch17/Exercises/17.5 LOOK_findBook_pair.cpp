...
/*
* Exercise 17.5
* Rewrite find_book to return pair taking an index and a pair of iters
*/
...
using index = std::vector<Sales_data>::size_type;
using pair_tuple = std::tuple<std::vector<Sales_data>::const_iterator,
...
*/
std::pair<index, pair_tuple> find_book(const vv& files, index in, 
	const std::string& book) {
	std::pair<index, pair_tuple> ret(in, std::make_tuple(
		files[in].cend(), files[in].cend()));
		...
		if (first_iter != second_iter) {
			ret.first = in;
			pair_tuple temp(first_iter, second_iter);
			ret.second = temp;
		}
...
void report_results(std::istream& in, std::ostream& os, const vv& files) {
	...
	while (in >> s && s != "q") {
		...
		//decltype(files[0].cend()) null_iter;
		std::vector < std::pair<index, pair_tuple>> trans;

		for (decltype(files.size()) i = 0; i != files.size(); ++i) {
			trans.push_back(find_book(files, i, s));
		}

		if (std::get<0>(trans[0].second) == files[0].cend()
			&& std::get<0>(trans[1].second) == files[1].cend()
			&& std::get<0>(trans[2].second) == files[2].cend()) {
		...
		for (decltype(files.size()) i = 0; i != files.size(); ++i) {
			if (std::get<0>(trans[i].second) != files[i].cend()) {
				os << "store " << trans[i].first << " sales: "
					<< std::accumulate(std::get<0>(trans[i].second),
						std::get<1>(trans[i].second),
...
int main() {
	...
	/*
	* Not sure if this is what looking for, took some time, when defined
	* default pair object it would return <NULL> if no new iters assigned
	* to the tuple, which couldn't be used unless with decltype but then
	* if a book was found couldn't compare iter to <NULL>, so better to
	* initialize it
	*/
	return 0;
}