...
/*
* Exercise 18.10, 18.11
* Write a program that uses the Sales_data addition operator on objects that
* have differing isbns, write two versions of the program one that handles
* the exception and one that does not, compare behavior so know what happens
* when an uncaught exception occurs
* Why is it important the what function doesn't throw
*/
...
int main() {
	...
	while (std::cin >> sd1 >> sd2) {
		...
		sum = sd1 + sd2; //abort called if not handled when handles, while
		//loop proceeds to next iteration

		/*
		* If what function does throw then either user doesn't get the message
		* if it's handled in what() function or it looks for a matching handler
		* which is not found so program exits, either way it won't accomplish
		* what it's designed to do which is give info on exception when the 
		* exception is supposed to be handled and allow for more execution after
		* handler if that's the intent
		*/
	}
	return 0;
}